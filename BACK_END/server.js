...
/**
 * Relatia intre modelele: Conference si Reviewer
 *
 * Descriere:
 * O conferinta poate avea mai multi revieweri.
 * Un reviewer poate fi alocat mai multor conferinte.
 */
Conference.belongsToMany(Reviewer, {
  through: ConferenceReviewer,
  foreignKey: "conferenceId",
  otherKey: "reviewerId",
});
Reviewer.belongsToMany(Conference, {
  through: ConferenceReviewer,
...
/**
 * GET /conferences/:idConference/reviewers
 *
 * Descriere:
 * Returneaza revieweri unei conferite.
 */
app.get("/conferences/:idConference/reviewers", async (req, res, next) => {
  try {
    const conference = await Conference.findByPk(req.params.idConference, {
      include: [Reviewer],
    });
...
app.delete("/organisers/:idOrganiser", async (req, res, next) => {
  try {
    ...
    const deletedCount = await Organiser.destroy({
      where: { id: idOrganiser },
    });
 ...
 *
 * Descriere:
 * Adauga review-erii pentru o conferinta existenta.
 */
app.post("/conferences/:conferenceId/reviewers", async (req, res, next) => {
  try {
    const conference = await Conference.findByPk(req.params.conferenceId);
    if (!conference) {
      ...
      const updatedConference = await Conference.findByPk(
